...
from . import get_parser, read, read_format, read_int, read_struct, read_until, wait
from .exceptions import ParseError
from .parser import Parser, Buffer
...
class Unit(abc.ABC):
    ...
    def parse(self, data: bytes, *, strict: bool = True):
        ...
        return Parser.new(self.get_value(), len(data)).parse(data, strict=strict)
...
class BinarySchemaMetaclass(type):
    ...
    def get_parser(cls, buffer: Buffer = None) -> Parser:
        return Parser(cls.get_value(), buffer)
...
class StructUnit(Unit):
    ...
    def get_value(self):
        return (yield from read_struct(self._struct))[0]
...
class LengthPrefixed(Unit):
    ...
    def get_value(self):
        ...
        parser = Parser.new(self._gen(), len(data))
